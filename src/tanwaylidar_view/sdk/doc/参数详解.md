...
- `IMUPort`：IMU数据端口号。目标设备（主机）监听IMU数据的端口号，默认为5900，没有IMU的雷达系列可以不关注该字段。
...
定义 `IMUData` 结构体用于存储惯性测量单元（IMU）的数据，**仅支持Duetto、TW360型号**，包含：
...
- `temperature`： IMU 的温度。
- `angular_velocity`：IMU 测量的角速度，分别对应 x、y、z 三个轴的角速度，单位rad/s。
- `linear_acceleration`： IMU 测量的线性加速度，分别对应 x、y、z 三个轴的线性加速度，单位g。
- `gyro_noise`：陀螺仪的噪声水平。
- `gyro_bias`：陀螺仪的偏差。
